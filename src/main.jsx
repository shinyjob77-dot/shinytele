...
import allergyTableHtml from "./provider_tools/disease_conditions/Allergy_table.html?raw";
import providerSignatureImageUrl from "./provider_tools/disease_conditions/provider-signature.png";
import prescriptionPadHtml from "./provider_tools/disease_conditions/telednpnow-prescription-pad.html?raw";
import referralFormHtml from "./provider_tools/disease_conditions/telednpnow-referral-form.html?raw";
...
const diseaseConditionTools = [
  {
    ...
    html: prescriptionPadHtml,
  },
  {
    title: "Referral Form",
    slug: "referral-form",
    html: referralFormHtml,
  },
  {
    title: "Acne Clinical Guide",
    slug: "acne-clinical-guide",
    html: acneClinicalGuideHtml,
  },
  {
    title: "Acne Differential Diagnosis and Treatment",
    slug: "acne-diagnosis",
    html: acneDiagnosisHtml,
  },
  {
  ...
  },
  {
    ...
    slug: "acne-intake-form",
    html: acneIntakeHtml,
  },
  {
    title: "Allergy Differential Diagnosis and Management",
    slug: "allergy",
    html: allergyHtml,
  },
  {
    title: "Allergy Diagnostic and Treatment Tool",
    slug: "allergy-automatic",
    html: allergyAutomaticHtml,
  },
  {
    title: "Allergy Diagnostic Tool",
    slug: "allergy-table",
    html: allergyTableHtml,
  },
];
...
function ProviderDiseaseConditionViewer() {
  const accessToken = new URLSearchParams(window.location.search).get("token");
  const [selectedSlug] = useState(() =>
    accessToken
      ? window.localStorage.getItem(`telednpnowProviderTool:${accessToken}`)
      : "",
  );
  const selectedTool = diseaseConditionTools.find(
    (tool) => tool.slug === selectedSlug,
  );
  const isAuthorized =
    window.localStorage.getItem("telednpnowProviderAuthorized") === "yes";
  const canOpenTool = isAuthorized && Boolean(selectedTool);

  useEffect(() => {
    if (accessToken) {
      window.localStorage.removeItem(`telednpnowProviderTool:${accessToken}`);
    }
  }, [accessToken]);

  if (!canOpenTool) {
    return (
      <main className="provider-viewer-shell">
        <section className="provider-viewer-panel">
          <h1>Provider Login Required</h1>
          <p>
            Please open disease condition tools from the Provider Login section
            on the TeleDNPnow home page. Direct links are not allowed.
          </p>
          <a href="/">Back to TeleDNPnow</a>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="provider-viewer-shell">
      <header className="provider-viewer-header">
        <div>
          <span className="section-kicker">Provider tool</span>
          <h1>{selectedTool.title}</h1>
        </div>
        <div className="provider-viewer-actions">
      ...
      <iframe
        className="provider-tool-frame"
        title={selectedTool.title}
        srcDoc={selectedTool.html.replaceAll(
          "__PROVIDER_SIGNATURE_IMAGE__",
          providerSignatureImageUrl,
        )}
      />
    </main>
  );
...
function App() {
  ...
  const [selectedDiseaseConditionTool, setSelectedDiseaseConditionTool] =
    useState("");
  ...
  const handleProviderLogin = (event) => {
    ...
    if (isAuthorized) {
      window.localStorage.setItem("telednpnowProviderAuthorized", "yes");
    }
  ...
  const openSelectedDiseaseConditionTool = () => {
    const selectedTool = diseaseConditionTools.find(
    ...
    if (selectedTool) {
      ...
      window.localStorage.setItem(
        `telednpnowProviderTool:${token}`,
        selectedTool.slug,
      );
      window.open(
        `/provider-tools/disease-conditions?token=${encodeURIComponent(token)}`,
        "_blank",
        "noopener,noreferrer",
      );
    }
  ...
  if (normalizedPath === "/provider-tools/disease-conditions") {
    return <ProviderDiseaseConditionViewer />;
  }
